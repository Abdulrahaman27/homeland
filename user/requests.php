...
<?php
    ...
    $requests = $conn->query("SELECT props.id AS id, props.name AS name, props.location AS location, props.image AS image, props.price AS price, props.beds AS beds, props.baths AS baths, props.sqft AS sqft, props.type AS type FROM props JOIN requests ON props.id = requests.prop_id
    ...
    <div class="site-blocks-cover inner-page-cover overlay" style="background-image: url(<?php echo BASE_URL; ?>/images/hero_bg_2.jpg);" data-aos="fade" data-stellar-background-ratio="0.5">
     ...
     <div class="container ">
        ...
        <?php if(count($props) > 0) : ?>
          ...
          <?php endforeach; ?>
        <?php else: ?>
            <div class="bg-success text-white  p-2 mx-auto">You don't have any requests for properties at the moment.</div>
            <?php endif; ?>
        </div>
      </div>
